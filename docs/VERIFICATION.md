...
- 실제 WebSocket 틱, 구독 종목 교체와 해당 종목 틱, 명시적 종료 후 새 연결의 ACK/틱 수신은 3회 모두 통과했다. 자동 단절 복구·장시간 세션·3계좌 동시 소켓 검증은 아니다.
- REST는 처음 두 실행에서 매매용 검증 거절. 두 번째 진단에서 가격/호가/범위는 유효하고 신선도 검사만 NO임을 확인했다. 세 번째 실행의 REST 검증은 통과했다. 신선도 기준을 낮추지 않았다.
- HTTP 429: 두 번째 실행의 2번 계좌 잔고, 세 번째 실행의 3번 계좌 손익에서 각 1건. 후속 별도 조회 성공을 원래 요청의 성공으로 바꾸지 않았다. 감속 보완은 후속 호출에만 적용하고 자동 재전송/토큰 재발급 없음.
- 일봉 원본 조회, OpenDART 공시·2025 연결재무 조회 통과. 수정주가 보증·뉴스 이용권한·전략 수익성은 검증하지 않았다.
- 모의 주문/취소와 실자금 주문은 발송하지 않았다. GroupExecutionSource 주문번호 대응 미구현과 연구 제공자 게이트 때문에 전략·그룹·파킹의 실제 자동주문 E2E는 미완료다. 가격 거리/주기·극값·전략·파킹·계좌 격리의 JVM 회귀를 실제 체결 증거로 대신하지 않는다.
- 최종 JVM 1,129개 통과(1,000 조합 + 기존 125 + 감속 경계 4), 조합 ID 각각 1회/실패·오류·skip 0 확인. `testDebugUnitTest lintDebug assembleDebug bundleRelease assembleDebugAndroidTest` 성공. 린트 오류 0/기존 버전 경고 13개. 계층·출처·문서 링크·diff 검사 통과.
- 일반 Android API 30 회귀: XML 고유 testcase 32개 중 31개 통과, 실제 키 opt-in 1개 의도적 제외, 실패/오류 0. Gradle 콘솔 33개 표시 대신 XML로 확인했다. 계좌 AAD/이력/미확인 원장, 조회 경계, 다운로드·통계·전략·파킹·시세 옵션·설정 UI, IO/취소/내보내기 실패, 암호문 훼손 검사를 포함한다. 기존 preview/글꼴/adb 권한 복원 및 신규 시험 설치 제거 성공. 확대 글꼴·실물 기기·장시간 서비스는 이번 요청에서 추가 실행하지 않았다.

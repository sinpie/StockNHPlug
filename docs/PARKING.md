...
기준일: 2026-09-20. 사용자가 지정한 종목으로 유휴 현금을 운용하는 기능입니다. 특정 종목 추천, 현금 등가성 또는 수익 보장을 의미하지 않습니다. 나무매직 저장소 URL을 확인하지 못했으므로 동일한 알고리즘이라고 주장하지 않습니다. 적응형 타겟 추적은 [별도 미구현 항목](PRICE_TRACKING.md)입니다.
...
- NHPlug 주문번호/체결번호의 공식 대응과 모의계좌 검증이 없어 자동주문 시작 잠금은 유지됩니다. 실제 파킹 주문은 실행하지 않았습니다.
...
- 실제 타겟 추적 후 실행은 나무매직 확인 전 미구현입니다. 현재 신선한 WebSocket 호가와 기존 그룹 조건으로 평가하고 IOC 주문 경계를 유지합니다.
- 장중 서비스 세션 안에서만 동작합니다. 네트워크/잔고/원장 오류와 미확인 주문은 안전 정지하며 자동 재전송하지 않습니다.

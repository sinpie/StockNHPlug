...
기록일: 2026-09-26. 요청 원문은 [USER_REQUESTS.md](USER_REQUESTS.md)를 먼저 읽는다. 이 파일은 앱 전체 구현 완료 보고가 아니다.

## 이번 요청 반영 상태

- 요청 31 로컬 디버깅·검증 완료: 공통 매수 가격 이력 무결성/최신성 검사, 정확한 30분 만료, 조정 OHLCV overflow 거절, 무효 미래 시세 캐시 오염 방지, 운영 종료 후 REST/WS 반영 차단. 신규 5개 실패를 수정 전 재현했고 신규 총 8개 포함 JVM 1,156개/Android 35개 통과(opt-in 1개 제외). 린트 오류 0/버전 경고 13, Debug APK·Release AAB·조합/계층/소스/문서 검사 성공. 기기 원상 복원 확인. 실제 API/주문/장시간 시험 및 외부 계약 게이트는 미완료 유지. DEBUGGING_REQUEST31.md/VERIFICATION.md 참고.

- 요청 30 로컬 디버깅·검증 완료(2026-09-26): 종료 lease의 대기 연결 재등록, 취소 비협조 가능수량 응답 후 주문, 예약 중 취소 후 전송, 분석 실패 후 이전 근거 잔존을 신규 JVM 4개로 수정 전 재현 후 수정. 내보내기 중복 콜백의 busy 해제도 보강. JVM 1,148개/Android 35개 통과(opt-in 1개 제외), 린트 오류 0/버전 경고 13, Debug APK·Release AAB·계층/출처/조합/문서 검사 성공. 기기 임시 설치 제거 및 이전 설정 복원. 실제 키/API/주문/장시간 운용은 이번에 실행하지 않았으며 앱 전체 외부 게이트는 유지. 상세는 DEBUGGING_2026_09_26.md/VERIFICATION.md.

- 요청 29 부분 구현·검증 완료, 외부 근거 대기: 공유 시세 연결, OpenDART 기업코드 자동 연결, 계좌별 분석 설정 암호화 저장/15분 자동 갱신 구현. JVM 1,144개 및 Android 34개 통과(opt-in 1개 제외), 빌드/린트/계층/소스/문서 검사 성공. 실제 공유 WS 3 lease 및 수정 후 DART 전용 진단 통과. 전체 모의 진단의 HTTP 429/초기 DART 실패와 중간 다운로드 시간 초과도 VERIFICATION.md에 기록. 실제 주문 대사/수정주가 계약/허가된 뉴스/휴장일 및 Play 외부 게이트는 COMPLETION.md 기준 미완료다. 잠금 제거로 완료 처리하지 않는다. 기기 임시 설치 제거, 기존 preview/폰트/adb 권한 복원 완료.

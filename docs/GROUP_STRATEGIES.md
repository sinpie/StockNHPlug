...
- 전략 최대 20개, 그룹 최대 100개, 그룹당 종목 최대 10개입니다. 현재 감시 합집합 앱 한도는 100종목, 동시 WS 앱 한도는 10종목이며 나머지는 적응형 REST를 사용합니다. 동일 종목은 한 번만 구독합니다. 설정 그룹 수와 동시에 연결 가능한 고유 종목 수는 다릅니다.
...
- 앱의 사용자가 시작한 foreground 세션 안에서만 평가합니다. 앱 자체의 시간 만료는 없고 최근 앱 목록 제거 후에도 계속 실행합니다. 알람/부팅 자동 실행/앱 종료 상태의 실행 보장은 없습니다.
...
물타기는 현재가 도달 전에도 추가매수/익절 제시가를 등록합니다. 전략 제시가와 실제 반전 트리거는 별도이며 [가격 추적](PRICE_TRACKING.md)을 따릅니다.

...
## 2026-09-20 추적 통신 변경
운영 호스트 허용은 공식 currentPrice 읽기와 시세 WS에 한정합니다. 주문 호스트·실거래 빌드 잠금은 유지합니다. 시세 연결 장애는 ACK 확인된 WS 또는 신선한 REST로만 복구하며 주문 API는 재시도하지 않습니다. 원본 NamuMagic의 비공개 코드/설정/키는 저장소와 CI에 포함하지 않습니다. 추적 극값은 메모리 전용이며 정지·계좌 전환 시 폐기합니다.

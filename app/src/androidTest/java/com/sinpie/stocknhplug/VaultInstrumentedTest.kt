...
class VaultInstrumentedTest {
    @Test fun roundTripCiphertextAndDeletion() {
        ...
        try {
            ...
            Assert.assertFalse(File(context.noBackupFilesDir,"test.enc").readText().contains("TEST_ONLY_SECRET"))
            vault.delete("test");Assert.assertNull(vault.read("test"))
        } finally {vault.delete("test")}
    }
    @Test fun tamperedCiphertextFailsClosed() {
        val context=InstrumentationRegistry.getInstrumentation().targetContext;val vault=SecureVault(context)
        try {
            vault.write("test",JSONObject().put("value","test"));val file=File(context.noBackupFilesDir,"test.enc")

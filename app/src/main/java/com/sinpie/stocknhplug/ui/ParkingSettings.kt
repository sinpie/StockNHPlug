...
fun ParkingSettingsCard(state: AppState, save: (StrategyBook) -> Unit) {
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text("현금 파킹", fontWeight = FontWeight.Bold)
        ...
        Surface(Modifier.fillMaxSize()) {
            Column(Modifier.padding(20.dp)) {
                ...
                Column(
                    ...
                    if (symbolLocked) Text("거래 기록이 있는 파킹종목은 변경할 수 없습니다. 끄면 기존 보유량을 자동 매도하지 않습니다.")
                    ...
                    Text("현재 NHPlug 체결 대사 검증 전 실제 자동주문은 잠겨 있습니다. 나무매직과 동일한 가격 추적은 아직 적용되지 않았습니다.")
                    if (error != null)
                        Text(

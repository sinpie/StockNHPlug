...
enum class DataSource {
    ...
    OPENDART,
    LICENSED_NEWS,
    LICENSED_ADJUSTED_PRICES,
}

enum class PermissionStatus {
    OFFICIAL_API_PERSONAL,
    CONTRACT_REQUIRED,
    DISABLED,
}
...
object SourceRegistry {
    val policies =
        listOf(
            SourcePolicy(
                DataSource.NHPLUG,
                PermissionStatus.OFFICIAL_API_PERSONAL,
            ...
            SourcePolicy(
                DataSource.LICENSED_NEWS,
                PermissionStatus.CONTRACT_REQUIRED,
                LocalDate.of(2026, 9, 19),
                "",
            ),
            SourcePolicy(
                DataSource.LICENSED_ADJUSTED_PRICES,
                PermissionStatus.CONTRACT_REQUIRED,
                LocalDate.of(2026, 9, 19),
                "",
            ),
...
/**
 * Only licensed, explicit factors. No split detection by price jumps; no fabricated dividend
 * factors.
 */
        ...
        require(
            actions.all {
                it.priceFactor.isFinite() &&
                    it.priceFactor > 0 &&
                    it.volumeFactor.isFinite() &&
                    it.volumeFactor > 0
            }
        )
        val known =
            actions.filter {
                !it.publishedAt.isAfter(asOf) &&
                    !it.exDate.isAfter(
                        asOf.atZone(com.sinpie.stocknhplug.domain.SEOUL).toLocalDate()
                    )
            }
        return raw.map { c ->
...
data class FinancialReport(
    ...
    val operatingMargin: BigDecimal?
        get() = ratio(operatingProfit, revenue)

    val debtRatio: BigDecimal?
        get() = ratio(liabilities, equity)

    private fun ratio(a: BigDecimal?, b: BigDecimal?) =
        if (a != null && b != null && b > BigDecimal.ZERO)
            a.multiply(BigDecimal(100)).divide(b, 2, java.math.RoundingMode.HALF_UP)
        else null
}

data class Disclosure(val receipt: String, val title: String, val date: LocalDate) {
    val url
        get() = "https://dart.fss.or.kr/dsaf001/main.do?rcpNo=$receipt"

    val risk
        get() = Regex("거래정지|상장폐지|감사의견|횡령|배임|회생|파산|유상증자|불성실공시").containsMatchIn(title)
}
...
/**
 * Intentionally no scraper: must be replaced only after a licensed provider is approved in
 * DATA_SOURCES.md.
 */
...
data class ResearchEvidence(
    ...
    val financials: FinancialReport?,
    val disclosures: List<Disclosure>,
    val news: List<NewsItem>,
    val disclosuresChecked: Boolean,
    val newsLicensed: Boolean,
    ...
    fun buyBlockers(now: Instant): List<String> = buildList {
        ...
        if (financials == null || financials.equity == null || financials.operatingProfit == null)
            add("재무지표 미확인")
        if (
            financials?.equity?.let { it <= BigDecimal.ZERO } == true ||
                financials?.operatingProfit?.let { it <= BigDecimal.ZERO } == true
        )
            add("재무 필터 미충족")
